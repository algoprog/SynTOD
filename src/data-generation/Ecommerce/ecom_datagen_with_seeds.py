'''
This script explicitly uses the gpt4 and gpt4-turbo (gpt-4-1106-preview) LLMs only for generation of the conversation.
Uses transition graph.
REQUIRED: correct openai api key 
...
def gemini_api(prompt, model=gpt_4_turbo, temperature=0.5, max_retries=64, max_tokens = 2000) :
...
class DataGenerator:
    ...
    def generate_response(self, intent, next_intent=None, prev_intent=None, args=None):
        ...
        response, used_tokens = chatgpt_api(prompt, model=model, temperature=temperature, max_retries=7,max_tokens = maximum_token_length)
        ...
        if json_format:
            try:
                ...
                response, used_tokens = chatgpt_api(prompt, model=gpt_4, max_retries=7) # gpt_4 # gpt_4_turbo
