...
def generate_conversations_nograph(limit=100):
    ...
    for product in tqdm(products):
        ...
        #     futures.append(executor.submit(generate_conversation_nograph, final_prmpt, product['id'])) 
    ...
    #     completed += 1
        '''
            #     prompt = """Simulate a conversation between human and AI that helps with recipes, allowed user intents are; search_recipe (user searches for specific recipe), suggest_recipe (user asks for recipe recommendation based on occasion, cuisine or preferences), more_results (shows more recipe results), select (selects a recipe from a list), show_ingredients, begin (starts the recipe), next_step, goto_step, repeat, finish_task (finish the recipe guidance), open_domain_qa, in_task_qa (qa related to recipe instructions), chitchat, set_timer. The conversation should start with introduction from the system, when user asks for recipes, the system provides 3 options for the user to choose. Use this jsonl format:

        # {"system": <text>}
        # {"user": <text>, "intent": ...}
        # ...

        # Here is the information about the recipe the user is going to select:\n""" + recipe_info
...
def get_cost_till_now() :
    ...
    total_cost = ((0.01 + 0.03)/2.0*(gpt4_turbo_tokens+ gpt_4_tokens))/1000 # (0.03+0.06)/2.0*gpt_4_tokens
...
def add_product_seed_id_cost_till_now(limit = 2000) :
    ...
    conversations = []
    with open('data/responses_nograph_ecom_gpt4turbo.jsonl', 'r') as file: # responses_nograph_ecom_3
        for line in file:
            d = json.loads(line)             
            conversations.append(d)
...
# generate_conversations_nograph(limit=2000)
# get_cost_till_now()
add_product_seed_id_cost_till_now()
